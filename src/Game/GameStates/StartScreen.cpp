...
StartScreen::StartScreen(const std::vector<std::string>& startScreenDescription, Game* pGame) : m_pGame(pGame),
    ...
    m_tankSpriteAnimator(m_tankSprite.first)
{
    ...
    unsigned int currentBottomOffset = STARTSCREEN_HEIGHT - 4 * BLOCK_SIZE;

    for (const auto& currentRow : startScreenDescription)
    {
